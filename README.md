...
- **Visual Designer**: activity palette, drag-and-drop canvas with nested containers (If / For Each / While / Try-Catch), a properties panel, variables and in/out arguments, undo/redo, design-time validation, JSON import/export, a test run on a live bot with a streaming log, and publishing versioned processes.
- **AI in the Designer**:
  - *Build with AI* generates a whole workflow from a plain-language description, or edits the current one. Output is checked against the workflow schema and the activity catalog, and any errors go back to Claude to fix.
...
- **AI at run time**:
  ...
  - *AI activities*: **AI Prompt**, **AI Extract Data** (structured output validated against a JSON Schema) and **AI Agent**. The agent is given a goal and decides which platform activities to call (browser, HTTP, files) until the goal is met.
...
- **Activities**: control flow, log, assign, delay, JavaScript, get asset, HTTP, JSON, files, and browser actions through Playwright (open, navigate, click, type, get text, wait, screenshot, close).
...
Browser activities need Playwright browsers on the agent machine: `pnpm --filter @zamtest/agent exec playwright install chromium`. To use a Chromium that is already installed, set `ZAMTEST_BROWSER_EXECUTABLE=/path/to/chrome` instead.
...
- **Where to set the key**:
  ...
  - Set it on each **bot agent** to enable self-healing and the AI activities.
...
```
packages/
  core/         Workflow schema (zod), activity catalog, expression language, execution engine
  activities/   Runtime implementations: system, data, files, browser (Playwright), AI
...
- Workflow expressions and the *Run JavaScript* activity run with the agent's permissions. Only let trusted automation developers publish processes.
